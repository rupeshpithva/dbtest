...
public class AccountsControllerTest {
	...
	public void transferAmount() throws Exception {
		...
		this.mockMvc.perform(get("/v1/accounts/transferAmount?fromAccountId="+fromAccount.getAccountId()+"&toAccountId="+toAccount.getAccountId()+"&amount="+amount.toPlainString()))
				.andExpect(status().isOk())
				.andExpect(content().string("Amount transfer complete"));
	...
	public void transferAmountInsufficientBalance() throws Exception {
		Account fromAccount = new Account("123", new BigDecimal("10000"));
		Account toAccount = new Account("xyz",new BigDecimal("20000"));
		BigDecimal amount = new BigDecimal("11000.0");
		this.accountsService.createAccount(fromAccount);
		this.accountsService.createAccount(toAccount);
		
		this.mockMvc.perform(get("/v1/accounts/transferAmount?fromAccountId="+fromAccount.getAccountId()+"&toAccountId="+toAccount.getAccountId()+"&amount="+amount.toPlainString()))
				.andExpect(status().isBadRequest())
				.andExpect(content().string("The account " + fromAccount.getAccountId() +  " has not sufficient balance"))
				;

	}
	
	@Test
	public void transferAmountNegativeBalance() throws Exception {
		Account fromAccount = new Account("123", new BigDecimal("10000"));
		Account toAccount = new Account("xyz",new BigDecimal("20000"));
		...
		this.mockMvc.perform(get("/v1/accounts/transferAmount?fromAccountId="+fromAccount.getAccountId()+"&toAccountId="+toAccount.getAccountId()+"&amount="+amount.toPlainString()))
				.andExpect(status().isBadRequest())
				.andExpect(content().string("The amount to be transffered can not be negative"))
				;
